...
df<-read.csv(file_id$input_csv$US_points, header=T, sep=",") %>% 
  mutate(PointNumber = as.numeric(PointNumber))

# carico caratteristiche match
df_match<-read.csv(file_id$input_csv$US_match, header=T, sep=",") %>% 
...
rk<-read.csv(file_id$input_csv$US_ranking,  header=T, sep=";")
...
rk_clean = rk %>%
  ...
  mutate(player = gsub('J. Antoni Munar Clar', 'J. Munar', player),
         ...
         player = gsub('A. De Minaur', 'A. de Minaur', player),
...
list_df = prep_df_fin(df_servizio_clean, torneo = 'US')
...
saveRDS(df_serveA, file = file_id$file_temp_service$US_service_A)
saveRDS(df_serveB, file = file_id$file_temp_service$US_service_B)
