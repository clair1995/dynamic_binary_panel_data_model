...
dfA<-readRDS(file_id$file_temp_service$AO_service_A) 
dfB<-readRDS(file_id$file_temp_service$AO_service_B) 
...
saveRDS(conv$df_finale, file = file_id$output_df$out_df_AO)
...
saveRDS(mod.rid, file = file_id$output_mod$out_mod_AO)
