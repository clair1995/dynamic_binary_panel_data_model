...
df<-read.csv(file_id$input_csv$AO_points, header=T, sep=";")

# carico caratteristiche match
df_match<-read.csv(file_id$input_csv$AO_match, header=T, sep=",")

# carico file per il ranking scaricato da On Court
rk<-read.csv(file_id$input_csv$AO_ranking,  header=T, sep=";")
...
rk_clean = rk %>%
  ...
  mutate(player = gsub('J. Antoni Munar Clar', 'J. Munar', player),
         ...
         player = gsub('M. Djokovic', 'J. Tipsarevic', player)
         )
...
# TROVO DF FINALE PER AUSTRALIAN OPEN -----------
...
# ottengo due serie storiche di punti, una in cui serve A e una in cui serve B
# serve_A-> serie storica con tutti i punti in cui ha servito A nei diversi match
# serve_B-> serie storica con tutti i punti in cui ha servito B nei diversi match
...
list_df = prep_df_fin(df_servizio_clean, torneo = 'AO')
...
saveRDS(df_serveA, file = file_id$file_temp_service$AO_service_A)
saveRDS(df_serveB, file = file_id$file_temp_service$AO_service_B)
