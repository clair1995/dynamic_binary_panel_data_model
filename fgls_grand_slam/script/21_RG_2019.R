...
df<-read.csv(file_id$input_csv$RG_points, header=T, sep=";")

# carico caratteristiche match
df_match<-read.csv(file_id$input_csv$RG_match, header=T, sep=",")

# carico file per il ranking scaricato da On Court
rk<-read.csv(file_id$input_csv$RG_ranking,  header=T, sep=";")
...
rk_clean = rk %>%
  ...
  mutate(player = gsub('J. Antoni Munar Clar', 'J. Munar', player),
         ...
         player = gsub('J. Ignacio Londero', 'J. Londero', player),
         player = gsub('S. Darcis', 'C. Stebe', player)
  )
...
list_df = prep_df_fin(df_servizio_clean, torneo = 'RG')
...
saveRDS(df_serveA, file = file_id$file_temp_service$RG_service_A)
saveRDS(df_serveB, file = file_id$file_temp_service$RG_service_B)
