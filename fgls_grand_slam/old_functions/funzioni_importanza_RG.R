...
# in seguito userò le markov chain per calcolarmi le diverse prob.
...
df_A_serve<-function(df_A, df_B) {
  ...
  df_serveA<-as.data.frame(cbind(ID_match, mom_partita, turno, yat, rel_qual, ass_qual,
                                 yat_1, delta11, delta12,
                                 dat, delta21, delta22,
                                 imp, delta31, delta32))
  
  df_serveA$servizioA<-df_A$giocA
  df_serveA<-na.omit(df_serveA)
  
  # trasformo variabili da char a numeriche
  for (i in c(3:14)) {
    
    df_serveA[,i]<-as.numeric(as.character(df_serveA[,i]))
...
df_B_serve<-function(df_A, df_B) {
  ...
  df_serveB<-as.data.frame(cbind(ID_match, mom_partita, turno, ybt, rel_qual, ass_qual,
                                 ybt_1, delta11, delta12,
                                 dat, delta21, delta22,
                                 imp, delta31, delta32))
  
  df_serveB$servizioB<-df_B$giocB
  
  df_serveB<-na.omit(df_serveB)
  
  for (i in c(3:14)) {
    
    df_serveB[,i]<-as.numeric(as.character(df_serveB[,i]))
...
# PROBABILITà di vincere un set partendo sullo (0,0) -----------
...
imp_pg<- function(ppoint_srv1, ppoint_srv2){
  ...
  #  tie-break sul 6-6 (non al 5 set)
  rownames(imp2)<-vett1
...
# s_tb è lo stato in cui mi trovo  in quel momento ( x es. sullo (0,0), o sul (2,1) )
...
imp_gs<-function(probA,probB) { #, gameA, gameB)  # probA e probB sono le probabilità di far punto a servizio
  ...
  # richiamo funzione che calcola % occorrenza t-esimo game
  # l_n<-prob_n_occurs(gameA,gameB) 
  # 
  ...
  # }
  # 
  # imp_gs1<-as.matrix(as.vector(round(mat_imp,digits=3))) # ho l'importanza di ogni game nel set
  # 
  # #creo num1 che identifica somma dei game -> game 1 ci sarà sempre, game 12 è meno probabile
  # num<-c(0:6)
  # num1<-NULL
  # for (i in 1:length(num)) {
  #   for (j in 1:length(num)){
  #     num1<-c(num1,sum(num[j],num[i]))
  #   }
  # }

  # creo matrice dove ho imp_gs * l_n ( vettore con prob che n-esimo game sia presente)
  # imp_fin<-matrix(0,nrow(imp_gs1), 1)
  # imp_fin[1,1]<-imp_gs1[1,1]
  # rownames(imp_fin)<-vett
  # 
  # for ( i in 2:nrow(imp_gs1)) {
  # 
  #   imp_fin[i,1]<-imp_gs1[i,1]*l_n[num1[i]]
  # 
  # }
  # 
  # 
  # imp_fin<-imp_fin[-which(imp_fin[,]==0),]
  # 
  # imp_fin<-as.matrix(imp_fin,49,1)
  ...
  for (k in 1:length(st_set)){
    ...
    for( i in 1:length(st_game)) {
      ...
      if (st_game[i]== "6-6") {
        ...
        #N.B se è il quinto set non ho tie-break al quinto set (uso imp.pg[[1]])
...
# ovviamente per questa funzione dovrò usare y_da_servizio-> i pti segnati da A o B da servizio
