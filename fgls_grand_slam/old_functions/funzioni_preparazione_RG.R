...
sistemo_df<- function(df) {
  
  game_A<-df$gameA
  game_B<-df$gameB
  
  pA1=as.numeric(as.character(df$P1Score))
  pB1=as.numeric(as.character(df$P2Score))
  ...
  df$PointServer[which(df$PointServer==1)]<-"p1"
  df$PointServer[which(df$PointServer==2)]<-"p2"
  
  # modifico variabile rank-> come fatto nell'articolo
  
  df$rankA<-8-log(df$rankA,base=2)
  df$rankB<- 8-log(df$rankB,base=2)
  
  riga<-NULL
  for(i in 1:length(unique(df$match_id))) {
    
    v<-rep(i, table(df$match_id)[[i]])
    riga<-c(riga, v)
  ...
  serve_A<- df[which(df$PointServer=="p1"),]
  # dataset in cui serve B
  serve_B<-df[which(df$PointServer=="p2"),]
