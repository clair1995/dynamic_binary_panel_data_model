...
giocatore<-function(df,df_match) {
  ...
  m1<-df_match$match_id
  m2<-unique(df$match_id)
  giocA<-NULL
  giocB<-NULL
  rA<-NULL
  rB<-NULL
  ...
  for(i in 1:nrow(df_match)) {
    if(m1[i]==m2[i]) {
      gA<-rep(p1[i],table(df$match_id)[[i]])
      gB<-rep(p2[i],table(df$match_id)[[i]])
      r1A<-rep(rkA[i],table(df$match_id)[[i]])
      r1B<-rep(rkB[i],table(df$match_id)[[i]])
    }
    giocA<-c(giocA,as.character(gA))
    giocB<-c(giocB,as.character(gB))
    rA<-c(rA,r1A)
    rB<-c(rB,r1B)
  }
  
  df$giocA<-as.character(giocA)
  df$giocB<-as.character(giocB)
...
puntiA_B<-function(x00) {
  ...
  for (riga in 1:righe)
  {
    ...
    # mi preparo y1 cioè il punto successivo a y0
    if (substr(x[i+1],1,1)=="*") 
    {
    y1=substr(x[i+1],2,6)
    } else 
...
# ranking messo a mano su excel dato che molti giocatori, la maggior parte ne è sprovvista

sistemo_df<- function(finale) {
  
  game_A<-finale$gameA
  game_B<-finale$gameB
  
  pA1=as.numeric(as.character(finale$P1Score))
  pB1=as.numeric(as.character(finale$P2Score))
  
  for (i in 1:length(pA1)) {
    if (is.na(pA1[i])) pA1[i]=50
    if (is.na(pB1[i]=="AD")) pB1[i]=50
  }
  point_A<-pA1
  point_B<-pB1
  ...
  n<-length(game_A)
  for ( i in 1:n) {
    if (game_A[i]+game_B[i]>=11) {
      
      if (game_A[i]==game_B[i]) {gA[i]=6;gB[i]=6 }
      if (game_A[i]-game_B[i]==1) {gA[i]=6;gB[i]=5}
      if (game_B[i]-game_A[i]==1) {gA[i]=5;gB[i]=6}
      
    }
    
    if ( (point_A[i] %in% c(7:14)) & (point_B[i] %in% c(7:14)) ) {
      
      if (point_A[i]==point_B[i]) {pA[i]=6;pB[i]=6 }
      if (point_A[i]-point_B[i]==1) {pA[i]=7;pB[i]=6}
      if (point_B[i]-point_A[i]==1) {pA[i]=6;pB[i]=7}
      if (point_A[i]-point_B[i]==2) {pA[i]=7;pB[i]=5}
      if (point_B[i]-point_A[i]==2) {pA[i]=5;pB[i]=7}
  ...
  finale$PointServer[which(finale$PointServer==1)]<-"p1"
  finale$PointServer[which(finale$PointServer==2)]<-"p2"
  
  # modifico variabile rank-> come fatto nell'articolo
  
  finale$rankA<-8-log(finale$rankA,base=2)
  finale$rankB<- 8-log(finale$rankB,base=2)
  
  riga<-NULL
  for(i in 1:length(unique(finale$match_id))) {
    
    v<-rep(i, table(finale$match_id)[[i]])
    riga<-c(riga, v)
  ...
  # dataset in cui batte giocatore A
  serve_A<- finale[which(finale$PointServer=="p1"),]
  # dataset in cui serve B
  serve_B<-finale[which(finale$PointServer=="p2"),]
  
  return( list( serve_A, serve_B) )
}
